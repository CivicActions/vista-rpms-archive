...
- Python 3.11 + docling>=2.0.0, docling-core (HybridChunker), qdrant-client[fastembed]>=1.12.0, google-cloud-storage>=2.0.0, psutil>=5.9.0, transformers (for tokenizer alignment) (004-docling-chunking)
- GCS (source + cache buckets), Qdrant (vector DB, named vectors) (004-docling-chunking)
...
## Recent Changes
- 004-docling-chunking: Added Python 3.11 + docling>=2.0.0, docling-core (HybridChunker), qdrant-client[fastembed]>=1.12.0, google-cloud-storage>=2.0.0, psutil>=5.9.0, transformers (for tokenizer alignment)
